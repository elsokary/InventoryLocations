define(['plugins/router', 'services/dataservice', 'config'], function (router, dataservice, config) {
    ...
    var cardDescriptionData = ko.observableArray([]);

    var selectedTransactionTypes = ko.observableArray([]);
    ...
    var transactionTypeDto = function (data) {
        var self = this;
    ...
    var editAssignToLocation = function (obj, e) {


        $('#editAssignToLocationModal').modal('show');
    };

    var saveAssignToLocation = function (obj, e) {
 
        var list = Enumerable.From(transactionTypes()).Where(function (x) { return Enumerable.From(selectedTransactionTypes()).Contains(x.id) }).ToArray();
        var obj = Enumerable.From(cardDescriptionData()).Where(function (x) { return Enumerable.From(selectedRows()).Contains(x.id) }).ToArray();


        dataservice.assignItemToLocation(list, obj).then(function (data) {
    ...
    var saveEditAssignToLocation = function (obj, e) {

         var obj = Enumerable.From(cardDescriptionData()).Where(function (x) { return Enumerable.From(selectedRows()).Contains(x.id) }).ToArray();
         
         dataservice.editAssignItemToLocation(locationId(), transactionTypesId(), obj).then(function (data) {

            $('#editAssignToLocationModal').modal('hide');
